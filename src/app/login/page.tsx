...
export default function LoginPage() {
  ...
  useEffect(() => { fetch("/api/setup").then((response) => response.json()).then((data) => { if (!data.complete) router.replace("/setup"); else setChecking(false); }).catch(() => setChecking(false)); }, [router]);
